...
const navItems = [
  ...
  { icon: CheckCircle, path: '/approvals', label: 'X Approvals' },
  { icon: MessageSquare, path: '/dm-approvals', label: 'DM Approvals' },
  { icon: GitBranch, path: '/dm-conversations', label: 'DM Pipeline' },
        ...
        <div className="flex flex-col items-center gap-1 flex-1">
          {navItems.map((item) => {
            const isActive = location.pathname === item.path ||
              (item.path === '/dm-conversations' && location.pathname.startsWith('/dm-conversations'));
